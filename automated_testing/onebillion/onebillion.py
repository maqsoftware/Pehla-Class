...
class testingOperation:  
    ...
    def __init__(self,udId,port,file,id):
        ...
        os.system("adb -s "+udId+" shell getprop > devices"+udId+".txt")
    ...
    # checks if element exists or not using its id return boolean
    ...
    def allow_permission(self,driver):
        ...
        for x in range(3):  # iter according to the number of permission needs to be allowed
            try:
                driver.save_screenshot(self.output+r"\output_t"+str(self.count)+r".png")    # based on different types of elements present on different devices
